use ::css::sass::{FormalArgs, Function, Name};
use ::css::value::{Number, Numeric, Rgba};
use ::css::*;
use std::sync::Arc;
        ...
        Function::builtin(
            ...
            Arc::new(|s| {
                let a = s.get("a")?.numeric_value().map_err(|v| {
                    SassError::bad_arg("a".into(), &v, "is not a number")
                })?;
                let b = s.get("b")?.numeric_value().map_err(|v| {
                    SassError::bad_arg("b".into(), &v, "is not a number")
                })?;
                if a.unit == b.unit || b.unit.is_none() {
                    ...
                    Err(SassError::error("Incopatible args"))
                }
            }),
        ),
    );
