...
impl NumValue {
    ...
    pub fn as_ratio(&self) -> Result<Rational, crate::sassengine::SassError> {
        match self {
            ...
            NumValue::BigRational(r) => {
                ...
                loop {
                    ...
                    denom /= 32;
                    if denom.is_zero() {
                        return Err(crate::sassengine::SassError::BadValue(
            ...
            NumValue::Float(r) => Ratio::approximate_float(*r)
                .ok_or_else(|| crate::sassengine::SassError::BadValue(r.to_string())),
        }
    }
}
...
fn debug_int_value() {
    assert_eq!(
        format!("{:#?}", crate::sassengine::sass::Value::scalar(17)),
        "Numeric(\
         \n    Numeric {\
         \n        value: Number 17 / 1,\
         \n        unit: UnitSet [],\
