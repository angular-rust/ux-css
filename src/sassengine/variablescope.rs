...
use crate::sassengine::sass::{Function, Item, Mixin, Name};
...
impl ScopeRef {
    ...
    pub fn eval_body(self, body: &[Item]) -> Result<Option<Value>, SassError>
    where
        Self: Sized,
    {
        for b in body {
            let result = match *b {
                ...
                Item::For {
                    ...
                    let range = crate::sassengine::value::ValueRange::new(
...
impl<'a> Scope {
    ...
    pub fn get(&self, name: &str) -> Result<Value, SassError> {
        match self.get_or_none(&name.into()) {
            Some(value) => Ok(value),
            None => Err(SassError::undefined_variable(name)),
        }
    }
...
pub mod test {
    ...
    pub fn do_evaluate_or_error(
        s: &[(&'static str, &str)],
        expression: &[u8],
    ) -> Result<String, super::SassError> {
        use super::{Scope, ScopeRef};
        use crate::sassengine::parser::value::value_expression;
