...
use crate::sassengine::css::{BodyItem, Rule};
use crate::sassengine::error::SassError;
use crate::sassengine::file_context::FileContext;
use crate::sassengine::parser::parse_imported_scss_file;
use crate::sassengine::sass::{self, FormalArgs, Function, Item, Mixin, Name};
use crate::sassengine::selectors::Selectors;
use crate::sassengine::value::ValueRange;
use crate::sassengine::ScopeRef;
use std::io::Write;
...
fn handle_item(
    ...
    match item {
        Item::Use(ref name, ref as_n) => {
            use crate::sassengine::sass::{get_global_module, UseAs};
            let name = name.evaluate(scope.clone())?.0;
            let do_use = |module: ScopeRef| -> Result<(), SassError> {
                match as_n {
                    UseAs::KeepName => {
                        let name = name
                            .rfind(|c| c == ':' || c == '/')
                ...
                file_context.find_file(&name)?
            {
                ...
                let items = parse_imported_scss_file(
                    ...
                    crate::sassengine::parser::code_span(b"").into(),
                )?;
                handle_body(
                    &items,
                    head,
                ...
                return Err(SassError::S(format!("Module {:?} not found", name)));
            }
        }
        ...
        Item::FunctionDeclaration(ref name, ref args, ref pos, ref body) => {
                ...
                || name == "url"
            {
                ...
                p.opt_back("@function ");
                return Err(SassError::InvalidFunctionName(p));
            }
        ...
        Item::Property(ref name, ref value) => {
            if let Some(rule) = rule {
                ...
                return Err(SassError::S("Global property not allowed".into()));
            }
        }
