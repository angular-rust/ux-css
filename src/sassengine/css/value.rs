use crate::sassengine::css::CallArgs;
use crate::sassengine::error::SassError;
use crate::sassengine::ordermap::OrderMap;
use crate::sassengine::output::{Format, Formatted};
use crate::sassengine::sass::Function;
use crate::sassengine::value::{Color, ListSeparator, Number, Numeric, Operator, Quotes};
...
impl Value {
    ...
    #[deprecated]
    pub fn integer_value(&self) -> Result<i64, SassError> {
        match self {
            &Value::Numeric(ref num, ..) => num
                .value
                .clone()
                .into_integer()
                .map_err(|_| SassError::bad_value("an integer", self)),
            v => Err(SassError::bad_value("a number", v)),
        }
    }
