...
use crate::sassengine::sass::{CallArgs, SassString};
use crate::sassengine::value::{ListSeparator, Number, Numeric, Operator, Quotes, Rgba};
...
impl Value {
    ...
    pub fn evaluate(&self, scope: ScopeRef) -> Result<css::Value, SassError> {
        ...
        match *self {
                ...
                if let Some(name) = name.single_raw() {
                    if let Some(f) = scope.get_function(&name.into()) {
                        f.call(scope.clone(), &args).map_err(|e| match e {
                            SassError::BadArguments(msg, decl) => {
                                SassError::BadCall(msg, pos.clone(), Some(decl))
                            }
                            e => SassError::BadCall(
                                e.to_string(),
                                pos.clone(),
            ...
            Value::Map(ref m) => {
                let items = m
                    ...
                    .collect::<Result<OrderMap<_, _>, SassError>>()?;
        ...
        match *self {
            ...
            Value::Color(ref rgba, ref name) => {
                if let Some(name) = name {
                    ...
                    crate::sassengine::value::Color::from(rgba.clone())
