macro_rules! name {
    ($name:ident) => {
        crate::sassengine::sass::Name::from_static(stringify!($name))
    };
    () => {
        // an empty name
        crate::sassengine::sass::Name::from_static("")
    };
}
...
macro_rules! one_arg {
    ...
    ($name:ident = $value:expr) => {{
        use $crate::sassengine::parser::parse_value_data;
        (
            stringify!($name).into(),
...
macro_rules! def {
    ($f:expr, $name:ident( $($arg:ident$(=$val:expr)* ),* ), $body:expr) => {{
        use crate::sassengine::sass::functions::{Functions, FormalArgs};
        use std::sync::Arc;
