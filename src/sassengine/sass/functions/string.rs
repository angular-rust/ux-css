use super::{get_integer, get_string, FunctionMap};
use crate::sassengine::{css::Value, value::Quotes, SassError, Scope};
...
pub fn create_module() -> Scope {
    ...
    def!(f, slice(string, start_at, end_at = b"-1"), |s| {
        ...
        if start_at <= end_at {
            ...
            Err(SassError::S(format!("Bad indexes: {}..{}", start_at, end_at)))
        }
    });
    ...
    def!(f, unique_id(), |_s| {
        lazy_static! {
            static ref CALL_ID: Mutex<u64> = Mutex::new(u64::from(std::process::id()) * 0xa01);
        };
        Ok(Value::Literal(
            format!("x{:x}", {
