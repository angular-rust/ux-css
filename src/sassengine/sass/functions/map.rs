use super::{SassError, FunctionMap, Name};
use crate::sassengine::css::Value;
use crate::sassengine::ordermap::OrderMap;
...
fn get_va_map(
    s: &Scope,
    name: Name,
) -> Result<OrderMap<Value, Value>, SassError> {
    match s.get(name.as_ref())? {
        Value::Map(m) => Ok(m),
...
fn set(s: &Scope) -> Result<Value, SassError> {
    let map = get_map(s, name!(map))?;
    let mut args = match s.get("args")? {
        Value::List(v, ..) => v,
        _ => return Err(SassError::error("Expected $args to contain a value")),
    };
    if let Some(value) = args.pop() {
        Ok(Value::Map(set_inner(map, &args, value)?))
    } else {
        Err(SassError::error("Expected $args to contain a key"))
    }
}
fn set_inner(
    ...
    if let Some((key, rest)) = keys.split_first() {
        ...
        Err(SassError::error("Expected $args to contain a value"))
    }
}
