use super::{
    check, get_checked, get_numeric, get_opt_check, SassError, FunctionMap, Scope,
};
use crate::sassengine::css::Value;
use crate::sassengine::output::Format;
use crate::sassengine::sass::Name;
use crate::sassengine::value::{Number, Numeric, Quotes, Rational, Unit, UnitSet};
    ...
    def_va!(f, hypot(number), |s| match s.get("number")? {
        Value::List(v, _, _) => {
            if let Some((first, rest)) = v.split_first() {
                ...
                Err(SassError::error("At least one argument must be passed"))
            }
        }
        ...
        v => Err(SassError::bad_arg(name!(number), &v, "is not a number")),
    });
...
fn get_radians(s: &Scope, name: &str) -> Result<f64, SassError> {
...
fn get_unitless(s: &Scope, name: &str) -> Result<f64, SassError> {
    get_checked(s, name.into(), |v| Ok(check::unitless(v)?.into()))
}

// Only used by hypot function, which treats arguments as unnamed.
fn as_numeric(v: &Value) -> Result<Numeric, SassError> {
    check::numeric(v.clone()).map_err(SassError::error)
}
...
fn find_extreme(v: &[Value], pref: Ordering) -> Result<Value, SassError> {
    find_extreme_inner(v, pref)?
        .ok_or_else(|| SassError::error("At least one argument must be passed"))
        .map(Into::into)
}
...
fn find_extreme_inner(
    v: &[Value],
    pref: Ordering,
) -> Result<Option<Numeric>, SassError> {
    if let Some((first, rest)) = v.split_first() {
        let va = check::numeric(first.clone()).map_err(SassError::error)?;
