...
use crate::sassengine::error::SassError;
use crate::sassengine::parser::SourcePos;
use crate::sassengine::sass::{FormalArgs, Name};
use crate::sassengine::value::{Numeric, Quotes};
use crate::sassengine::{sass, Scope, ScopeRef};
...
type BuiltinFn = dyn Fn(&Scope) -> Result<Value, SassError> + Send + Sync;
...
impl Function {
    ...
    fn do_eval_args(
        ...
        def: ScopeRef,
        args: &CallArgs,
    ) -> Result<ScopeRef, SassError> {
        self.args.eval(def, args).map_err(|e| match e {
            sass::ArgsError::Eval(e) => e,
            ae => SassError::BadArguments(ae.to_string(), self.pos.clone()),
        })
    }
}
...
trait CheckedArg<T> {
    fn named(self, name: Name) -> Result<T, SassError>;
}
impl<T> CheckedArg<T> for Result<T, String> {
    fn named(self, name: Name) -> Result<T, SassError> {
        self.map_err(|e| SassError::BadArgument(name, e))
    }
}

fn get_checked<T, F>(s: &Scope, name: Name, check: F) -> Result<T, SassError>
where
...
fn get_numeric(s: &Scope, name: &str) -> Result<Numeric, SassError> {
    get_checked(s, name.into(), check::numeric)
}

fn get_integer(s: &Scope, name: Name) -> Result<i64, SassError> {
...
fn get_string(
    s: &Scope,
    name: &'static str,
) -> Result<(String, Quotes), SassError> {
...
mod check {
    ...
    use crate::sassengine::value::{Number, Numeric, Quotes};
...
fn test_rgb() -> Result<(), Box<dyn std::error::Error>> {
    use crate::sassengine::parser::code_span;
    use crate::sassengine::parser::formalargs::call_args;
    use crate::sassengine::value::Rgba;
...
#[cfg(test)]
use crate::sassengine::variablescope::test::do_evaluate;
