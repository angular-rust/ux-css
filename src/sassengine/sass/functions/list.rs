use super::{check, get_checked, get_integer, SassError, FunctionMap, Name};
...
fn list_index(n: i64, list: &[Value], name: Name) -> Result<usize, SassError> {
    let len = list.len();
    rust_index(n, len, name)
}

fn rust_index(n: i64, len: usize, name: Name) -> Result<usize, SassError> {
    if n > 0 && n as usize <= len {
        ...
        Err(SassError::BadArgument(name, msg))
    }
}
