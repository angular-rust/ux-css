use super::{
    ...
    nospecial_value, SassError, FunctionMap, Name,
};
use crate::sassengine::css::{CallArgs, Value};
use crate::sassengine::output::Format;
use crate::sassengine::value::{ListSeparator, Quotes, Rational, Rgba};
use crate::sassengine::Scope;
use num_traits::{one, Zero};

fn do_rgba(fn_name: &str, s: &Scope) -> Result<Value, SassError> {
    let a = s.get("alpha")?;
    let red = s.get("red")?;
...
pub fn values_from_list(
    ...
    use crate::sassengine::value::Operator::Div;
...
fn to_channel(v: &Value, name: Name) -> Result<Option<Rational>, SassError> {
        ...
        |s| match s.get("color")? {
            ...
            col => {
                ...
                if w == one() {
                    ...
                    Err(SassError::error("Only one argument may be passed to the plain-CSS invert() function"))
                }
            }
        }
    );
