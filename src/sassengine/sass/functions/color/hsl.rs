...
use super::{
    ...
    make_call, nospecial_value, to_rational_percent, SassError, FunctionMap,
};
use crate::sassengine::css::Value;
use crate::sassengine::output::Format;
use crate::sassengine::value::{Hsla, Numeric, Rational, Unit};
use crate::sassengine::Scope;
use num_traits::{one, zero};

fn do_hsla(fn_name: &str, s: &Scope) -> Result<Value, SassError> {
    let a = s.get("alpha")?;
    let hue = s.get("hue")?;
