use super::{SassError, FunctionMap};
use crate::sassengine::css::Value;
use crate::sassengine::parser::input_span;
use crate::sassengine::parser::selectors::{selector, selector_part, selectors};
use crate::sassengine::selectors::{Selector, SelectorPart, Selectors};
use crate::sassengine::value::{ListSeparator, Quotes};
use crate::sassengine::{ParseError, Scope};
...
fn parse_selectors(v: Value) -> Result<Selectors, SassError> {
    match v {
        Value::List(v, s, _) => {
...
fn check_selector(v: &Value) -> Result<Selector, SassError> {
    match v {
        Value::List(v, _, _) => Ok(Selector(v.iter().fold(
            Result::<_, SassError>::Ok(vec![]),
            |a, v| {
                let mut a = a?;
                if !a.is_empty() {
...
fn check_selector_part(v: &Value) -> Result<SelectorPart, SassError> {
    match v {
        Value::Literal(s, _) => {
...
fn parse_selector(s: &str) -> Result<Selector, SassError> {
    Ok(ParseError::check(selector(input_span(s.as_bytes())))?)
}

fn bad_selector(v: &Value) -> SassError {
    SassError::bad_value(
