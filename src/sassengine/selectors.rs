...
use super::{SassError, ParseError, ScopeRef};
use std::fmt;
use std::io::Write;
...
impl Selectors {
    ...
    pub fn eval(&self, scope: ScopeRef) -> Result<Selectors, SassError> {
        let s = Selectors::new(
            self.s
                .iter()
                .map(|s| s.eval(scope.clone()))
                .collect::<Result<Vec<_>, SassError>>()?,
        );
        ...
        use super::parser::input_span;
        use super::parser::selectors::selectors;
...
impl Selector {
    ...
    fn eval(&self, scope: ScopeRef) -> Result<Selector, SassError> {
        self.0
            .iter()
...
impl SelectorPart {
    ...
    fn eval(&self, scope: ScopeRef) -> Result<SelectorPart, SassError> {
        match *self {
            SelectorPart::Attribute {
                ref name,
                ref op,
