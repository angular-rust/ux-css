...
impl<'a> Stream<'a> {
    ...
    #[inline]
    pub fn curr_byte(&self) -> Result<u8, CssError> {
        if self.at_end() {
            return Err(CssError::UnexpectedEndOfStream);
        }
    ...
    #[inline]
    pub fn next_byte(&self) -> Result<u8, CssError> {
    ...
    pub fn consume_byte(&mut self, c: u8) -> Result<(), CssError> {
        if self.curr_byte()? != c {
            return Err(CssError::InvalidByte {
                expected: c,
    ...
    #[inline]
    pub fn skip_spaces_and_comments(&mut self) -> Result<(), CssError> {
        self.skip_spaces();
    ...
    pub fn consume_ident(&mut self) -> Result<&'a str, CssError> {
        let start = self.pos();
        ...
        if let Some(c) = iter.next() {
            if c.is_name_start() {
                ...
                return Err(CssError::InvalidIdent(self.gen_text_pos_from(start)));
            }
        }
    ...
    pub fn consume_string(&mut self) -> Result<&'a str, CssError> {
    ...
    pub fn skip_comment(&mut self) -> Result<(), CssError> {
        let start = self.pos();
        self.skip_comment_impl()
            .map_err(|_| CssError::InvalidComment(self.gen_text_pos_from(start)))?;
        Ok(())
    }

    fn skip_comment_impl(&mut self) -> Result<(), CssError> {
        self.consume_byte(b'/')?;
        self.consume_byte(b'*')?;
