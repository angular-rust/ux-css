...
///
/// ```
/// use ::css::{SelectorTokenizer, SelectorToken};
///
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.stream.curr_byte_unchecked() {
            b'*' => {
                if !self.after_combinator {
                    self.finished = true;
                    return Some(Err(CssError::UnexpectedSelector));
                }
            ...
            b'[' => {
                ...
                let op = match try2!(self.stream.curr_byte()) {
                    ...
                    _ => {
                        self.finished = true;
                        return Some(Err(CssError::InvalidAttributeSelector));
                    }
                };
            ...
            b':' => {
                ...
                if ident == "lang" {
                    ...
                    if lang.is_empty() {
                        self.finished = true;
                        return Some(Err(CssError::InvalidLanguagePseudoClass));
                    }
            ...
            b'>' => {
                if self.after_combinator {
                    self.after_combinator = false;
                    self.finished = true;
                    return Some(Err(CssError::UnexpectedCombinator));
                }
