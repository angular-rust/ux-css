...
// #![doc(html_root_url = "https://docs.rs/simplecss/0.2.0")] // SOME WRONG
...
impl fmt::Display for CssError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            CssError::UnexpectedEndOfStream => {
                write!(f, "unexpected end of stream")
            }
            CssError::InvalidIdent(pos) => {
                write!(f, "invalid ident at {}", pos)
            }
            CssError::InvalidComment(pos) => {
                write!(f, "invalid comment at {}", pos)
            }
            CssError::InvalidValue(pos) => {
                write!(f, "invalid value at {}", pos)
            }
            CssError::InvalidByte { expected, actual, pos } => {
                write!(f, "expected '{}' not '{}' at {}",
                       expected as char, actual as char, pos)
            }
            CssError::SelectorMissing => {
                write!(f, "selector missing")
            }
            CssError::UnexpectedSelector => {
                write!(f, "unexpected selector")
            }
            CssError::UnexpectedCombinator => {
                write!(f, "unexpected combinator")
            }
            CssError::InvalidAttributeSelector => {
                write!(f, "invalid or unsupported attribute selector")
            }
            CssError::InvalidLanguagePseudoClass => {
...
fn consume_statement<'a>(s: &mut Stream<'a>, rules: &mut Vec<Rule<'a>>) -> Result<(), CssError> {
    if s.curr_byte() == Ok(b'@') {
        s.advance(1);
        consume_at_rule(s)
    } else {
...
fn consume_at_rule(s: &mut Stream) -> Result<(), CssError> {
...
fn consume_rule_set<'a>(s: &mut Stream<'a>, rules: &mut Vec<Rule<'a>>) -> Result<(), CssError> {
    ...
    while s.curr_byte()? == b',' || start_rule_idx == rules.len() {
        ...
        let (selector, offset) = selector::parse(s.slice_tail());
        s.advance(offset);
        s.skip_spaces();
...
fn consume_declarations<'a>(s: &mut Stream<'a>) -> Result<Vec<Declaration<'a>>, CssError> {
...
///
/// ```
/// use ::css::{DeclarationTokenizer, Declaration};
///
...
fn consume_declaration<'a>(s: &mut Stream<'a>) -> Result<Declaration<'a>, CssError> {
    ...
    if value.is_empty() {
        return Err(CssError::InvalidValue(s.gen_text_pos_from(start)));
    }
