...
impl<'a, 'input: 'a> XmlNode<'a, 'input> {
    fn select(&self, text: &str) -> Option<roxmltree::Node<'a, 'input>> {
        let selectors = css::Selector::parse(text)?;
...
impl css::Element for XmlNode<'_, '_> {
    ...
    fn attribute_matches(&self, local_name: &str, operator: css::AttributeOperator) -> bool {
    ...
    fn pseudo_class_matches(&self, class: css::PseudoClass) -> bool {
        match class {
            css::PseudoClass::FirstChild => self.prev_sibling_element().is_none(),
